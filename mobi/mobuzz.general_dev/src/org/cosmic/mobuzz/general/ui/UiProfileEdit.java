...
public class UiProfileEdit extends Activity {
		...
		but_logout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isNetworkAvailable()) {
					...
					new downloadData(ldata, -1).execute();
				} else {
					popdNoInternet();
				}
			}
	...
	private void getProfieData() {
		...
		try {
			obj.putOpt("user", pref.getString("username", "defv")); //Insert a dummy-name to avoid passing null value. 4-character username is not possible in registration.
		...
		if (isNetworkAvailable()) { //Check the network status
			new downloadData(data, 0).execute();
		} else {
			popdNoInternet();
		}
	}
	...
	private void updateProfile() {
		...
		String userdata[] = { userName, name, email, cNomber, language, residence, oldPsswrd, newPasswrd, time_stamp, uudid };
		...
		if (isNetworkAvailable()) { //Check the network status
			new downloadData(editdata, 1).execute();
		} else {
			popdNoInternet();
		}
	}
	...
	private boolean validate() {
		if (nameValidate(name, edt_fullname) && emailValidate(email, edt_email) && contactValidate(cNomber, edt_contactno)) {
			if (spinnerValidate(residence, but_residence) && spinnerValidate(language, but_language)) {
	...
	public void updateViews(String result, int action) {
		...
		try {
				...
				if (result.contains("{'status':")) { //In order to support backward-compatibility
					...
					if (GlobalMethods.validateString(plp.getStatus())) {
						...
						if (("error_net_connection").equals(plp.getStatus().toLowerCase(Locale.ENGLISH))) {
							...
							popdRequestFailed(getString(R.string.msg_connect_no_title1), getString(R.string.msg_connect_no_para1), action);
						}
						else if (("error_net_other").equals(plp.getStatus().toLowerCase(Locale.ENGLISH))) {
						
							popdRequestFailed(getString(R.string.msg_connect_no_title1), getString(R.string.msg_connect_no_para2), action);
						}
						else if (("error_db_params").equals(plp.getStatus().toLowerCase(Locale.ENGLISH))) {
							
							popdRequestFailed(getString(R.string.msg_response_unexpected_title2), getString(R.string.msg_response_unexpected_para2), action);
							...
							popdReauth(getString(R.string.msg_response_reauthentication_title1), getString(R.string.msg_response_reauthentication_para1), action);
						} else if (("authentication_expired").equals(plp.getStatus().toLowerCase(Locale.ENGLISH))) {
							
							popdReauth(getString(R.string.msg_response_reauthentication_title2), getString(R.string.msg_response_reauthentication_para2), action);
						} else {
							
							popdRequestFailed(getString(R.string.msg_response_unexpected_title1), getString(R.string.msg_response_unexpected_para1), action);
						}
					...
					if (data[0].equals("profiledata")) { //Get server profile data
						...
						but_residence.setText(data[5].trim());
						but_language.setText(data[6].trim());

						but_residence.setTextColor(getResources().getColor(R.color.theme_ash));
			...
			popdRequestFailed(getString(R.string.msg_request_fail_title1), getString(R.string.msg_request_fail_para1), action);
			if(GlobalVariables.PRINT_DEBUG)
			{
				Log.e(TAG, " " + ex.getMessage());
			}
	...
	private class downloadData extends AsyncTask<String, Void, String> {
		...
		String[] params;
		int action; // 0:download profile data, 1:update profile data, -1:sign out
		
		public downloadData(String[] params, int action)
		{
			this.params = params;
			this.action = action;
		}
	...
	private void popdListLanguage() { //Language list
		...
		btn_msg_no.setText(R.string.msg_common_cancel);
	...
	private void popdListrResidence() { //Residence list
	...
	private void popdRequestFailed(String title, String content, int action) {
	...
	private void popdReauth(String title, String content, int action) {
