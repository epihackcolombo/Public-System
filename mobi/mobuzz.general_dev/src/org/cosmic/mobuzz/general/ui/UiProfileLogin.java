...
public class UiProfileLogin extends Activity {
	...
	private Button signinButton, registeredUserButton, anonymousUserButton;
	private TextView registerTextView, forgotpwordTextView;
	private String url_profile_login, time_stamp="";
	private TableLayout tableLayoutSignIn, tableLayoutControlSignIn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		anonymousUserButton = (Button) findViewById(R.id.anonymousUserButton);
		registerTextView = (TextView) findViewById(R.id.registerTextView);
		forgotpwordTextView = (TextView) findViewById(R.id.forgotpwordTextView);
		
		tableLayoutSignIn = (TableLayout) findViewById(R.id.tableLayoutSignIn);
		tableLayoutControlSignIn = (TableLayout) findViewById(R.id.tableLayoutControlSignIn);
		linearLayoutSignIn = (LinearLayout) findViewById(R.id.linearLayoutSignIn);

		usernameET.setTypeface(font);
		passwordET.setTypeface(font);
		signinButton.setTypeface(font);
		registeredUserButton.setTypeface(font);
		anonymousUserButton.setTypeface(font);
		registerTextView.setTypeface(font);
		forgotpwordTextView.setTypeface(font);

		int position = usernameET.getText().length(); //Position the cursor
		Editable editObj = usernameET.getText();
		...
		//To check whether this call is from after login to the app
		isSiblingCalled = getIntent().getBooleanExtra("isSiblingCalled", false);
		System.out.println("isBundleCall: "+isSiblingCalled);
	...
	public void onResume() {
		...
		tableLayoutSignIn.setVisibility(View.GONE);
		linearLayoutSignIn.setVisibility(View.GONE);
		tableLayoutControlSignIn.setVisibility(View.VISIBLE);
	...
	private void registeredUserButton_Click() {
		registeredUserButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				tableLayoutSignIn.setVisibility(View.VISIBLE);
				linearLayoutSignIn.setVisibility(View.VISIBLE);
				tableLayoutControlSignIn.setVisibility(View.GONE);
			}
		});
	}
	...
	private void anonymousUserButton_Click() {
		...
		anonymousUserButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				...
				try {
					...
					if(GlobalMethods.validateString(plp.getStatus()))
					{
						...
						if(("ok").equals(plp.getStatus().toLowerCase()))
						{
							...
							if(!isSiblingCalled)
							{
								isSiblingCalled = false;
								startActivity(new Intent(context, UiMain.class));
							}
							finish();
						}
						else if (("error_net_connection").equals(plp.getStatus().toLowerCase(Locale.ENGLISH))) {
						...
						else if(("authentication_blocked").equals(plp.getStatus().toLowerCase())) //User profile is blocked
						{
