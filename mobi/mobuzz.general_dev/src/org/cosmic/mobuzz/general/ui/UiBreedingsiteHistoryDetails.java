...
public class UiBreedingsiteHistoryDetails extends android.support.v4.app.FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Draw CMC jurisdiction on google map
	...
	public void onResume() {
		...
		//If image is not in image-view and image is available, check whether image is already saved to the disk
		if (image > 0) {
	...
	private void but_actimage_Click()
	{
		but_actimage.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				setImage("actual"); // get the actual size image
			}
		});
	}

	
	//get the image from the disc, if available
	...
	private void setGalleryImage(String imagePath)
	{
		if (GlobalMethods.validateString(imagePath)) {
			...
			image = 0; //To stop downloading image again
		}
	}
				...
				try
				{
					...
					else
					{
			    		 ...
			    		 {			    	
						    ...
						    if(GlobalMethods.validateString(res)) //check the returned value to determined whether save is success
						    {
	...
	private void setUpMap() {
		...
		if (mMap == null) {
			...
			if (latLng != null) {
				...
				mMap.addMarker(new MarkerOptions().position(latLng).icon(BitmapDescriptorFactory.fromResource(R.drawable.logo_icon_marker_loc))); //Set the initial camera position for the map
				mMap.addPolygon(new PolygonOptions().addAll(cmcRegions.getCMCRegion()).fillColor(Color.parseColor(GlobalVariables.COLOR_REGION_OVERLAY))).setStrokeWidth(5); //Draw the CMC jurisdiction on the google map
	...
	public void responseToResult(String result) {
		...
		try {
			...
			popdRequestFailed(getString(R.string.msg_request_fail_title1), getString(R.string.msg_request_fail_para1)); //Show a common error message
