...
if (!empty($isvaliduser)) {
    ...
    $result = mysqli_query($con, "SELECT password FROM public_users WHERE username='$user'");
    ...
    if ($s_password == $password) {
        mysqli_query($con, "UPDATE public_users SET email='$email', full_name = '$name', residence = '$residence', language = '$language', contact_no = '$m_mobile' WHERE username='$user'");

        if ($newpassword != null) {
            mysqli_query($con, "UPDATE public_users SET password = '$newpassword' WHERE username='$user'");
        }

        echo PROF_EDIT_SUCCESS;
    } else {

        echo PROF_EDIT_ERROR_AUTH;
    }
