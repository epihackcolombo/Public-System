...
function get_data($user, $strfrom, $strto)
{
    ...
    if ($comments->num_rows <= 0) {
        ...
        if (!empty($ward_no)) {
            ...
            $sql_condition = "WHERE (verified=0) AND (user_status=1) AND (date_time BETWEEN '$strfrom' AND '$strto') AND (";
			...
			$sql_ward_data = "SELECT * FROM vw_public_data $sql_condition";
            ...
            while ($row2 = mysqli_fetch_array($result)) {
				...
				if (strcasecmp('male', $row2['gender']) == 0) {
					$fullname = "Mr. ".$row2['full_name'];
				}
				else if(strcasecmp('female', $row2['gender']) == 0) {
					$fullname = "Ms. ".$row2['full_name'];
				}
				else
				{
					$fullname = "";
				...
				if (empty($row2['img_path'])) { // no image
                    $jresult .= "{ 'id':'$row2[id]', 'image':0, 'imagepath':'', 'gps':'$row2[gps]' , 'username':'$row2[username]', 'address':'$row2[address]', 'date':'$row2[date_time]', 'ward':'$row2[ward]', 'remarks':'$row2[remarks]', 'verified':$row2[verified], 'assessment':'$row2[assessment]', 'cmcmessage':'$row2[cmcmessage]', 'fullname':'$fullname', 'contnumber':'$row2[contact_no]'},";
                } else { //has image
                    $jresult .= "{ 'id':'$row2[id]', 'image':1, 'imagepath':'".strrchr($row2['img_path'],"/")."', 'gps':'$row2[gps]' , 'username':'$row2[username]', 'address':'$row2[address]', 'date':'$row2[date_time]', 'ward':'$row2[ward]', 'remarks':'$row2[remarks]', 'verified':$row2[verified], 'assessment':'$row2[assessment]', 'cmcmessage':'$row2[cmcmessage]', 'fullname':'$fullname', 'contnumber':'$row2[contact_no]'},";
                }
            }
