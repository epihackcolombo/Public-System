...
function get_data($image, $gps, $address, $date_time, $user, $time_stamp, $remarks, $assess) {
    ...
    $au_sql = "SELECT * FROM public_users WHERE username = '$user'";

    $au_comments = $mysqli->query($au_sql);

    if ($au_comments == false) {
        close_db($mysqli);
    ...
    $sql = "INSERT INTO public_data (gps, username, address, verified, ward, date_time, remarks, assessment, img_path)
		VALUES ('$gps', '$user', '$address', 0, $ward, '$date_time', '$remarks', '$assess','$image_path')";
...
try {
    ...
    if (is_object(json_decode($data))) {
        ...
        if (property_exists($json, 'image')) {
            $image = $json->{'image'};
			//$image=$FBI->xss_clean($image); //this is not necessary, since the data is directly saved and read and send back to the mobile. The processing of this data is also after user validation.
        }
